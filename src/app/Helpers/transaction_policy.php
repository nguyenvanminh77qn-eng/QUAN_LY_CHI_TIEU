...
function validateTransaction($userId, $data, $editingId = null) {
    ...
    if ($type === 'expense') {
        ...
        $categoryLimitError = checkCategoryLimit($userId, (int)($data['category_id'] ?? 0), $price);
        if ($categoryLimitError !== null) {
            $errors['category_limit'] = $categoryLimitError;
            return ['valid' => false, 'errors' => $errors, 'warnings' => $warnings];
        }
    }
...
function validateBasicFields($data) {
    ...
    } elseif (mb_strlen($description, 'UTF-8') > 100) {
...
function purgeArchivedTransactions($userId) {
    $cutoffDate = date('Y-m-d', strtotime('-730 days'));

    $countResult = getOne(
        "SELECT COUNT(*) as cnt FROM transaction WHERE user_id = :id AND transaction_date < :cutoff AND is_archived = 1",
        ['id' => $userId, 'cutoff' => $cutoffDate]
    );
    $purgedCount = $countResult ? (int) $countResult['cnt'] : 0;

    if ($purgedCount > 0) {
        query(
            "DELETE FROM transaction WHERE user_id = :id AND transaction_date < :cutoff AND is_archived = 1",
            ['id' => $userId, 'cutoff' => $cutoffDate]
        );
    }
...
function checkCategoryLimit($userId, $categoryId, $newAmount) {
    if ($categoryId <= 0) return null;

    $limit = getOne(
        "SELECT max_amount FROM category_limit WHERE user_id = :uid AND category_id = :cid",
        ['uid' => $userId, 'cid' => $categoryId]
    );
    if (!$limit || (float)$limit['max_amount'] <= 0) return null;

    $maxAmount = (float)$limit['max_amount'];

    if ($newAmount > $maxAmount) {
        $maxFormatted = number_format($maxAmount, 0, ',', '.');
        $newFormatted = number_format($newAmount, 0, ',', '.');
        return "Danh mục này chỉ cho phép giao dịch tối đa {$maxFormatted}đ. "
             . "Số tiền {$newFormatted}đ vượt quá hạn mức cho phép.";
    }
...
/**
 * Kiểm tra xem category có hạn mức được đặt chưa
 * Trả về error nếu chưa đặt hạn mức
 */
function checkCategoryLimitExists($userId, $categoryId) {
    if ($categoryId <= 0) return null;

    $limit = getOne(
        "SELECT id, max_amount FROM category_limit WHERE user_id = :uid AND category_id = :cid",
        ['uid' => $userId, 'cid' => $categoryId]
    );

    if (!$limit || (float)$limit['max_amount'] <= 0) {
        $category = getOne(
        ...
        return "not_set|Danh mục \"{$categoryName}\" chưa có hạn mức. Vui lòng đặt hạn mức trước khi thêm giao dịch.";
    }
