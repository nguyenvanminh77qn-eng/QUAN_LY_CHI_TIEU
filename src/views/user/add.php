...
$data = getCachedCategories('id');
$userId = getSession('id');
createDefaultWallets($userId);
$wallets = getWallets($userId);
layout("header", ["title" => "Thêm Chi Tiêu","css" => ["layout/sidebar", "pages/user/add"]]);
...
if (!empty($errors)) {
    ...
    if (!empty($errors['wallet'])) $topLevelErrors[] = $errors['wallet'];
    ...
    <main class="main-content">
        <header class="top-header">
            <div class="header-left">
                ...
                <div><span class="subtitle">DIGITAL CURATOR</span><h1>Thêm Chi Tiêu</h1></div>
            </div>
                    ...
                    <div class="form-row" style="display:flex;gap:15px;flex-wrap:wrap;">
                        <div class="expense-form__group" style="flex:2;min-width:180px;">
                            ...
                            <div class="expense-form__input-wrapper">
                                <input type="text" id="amount" class="expense-form__input" placeholder="Nhập số tiền..." name="price" value="<?= htmlspecialchars($displayPrice) ?>" inputmode="numeric" required>
                    ...
                    <div class="form-row" style="display:flex;gap:15px;flex-wrap:wrap;">
                        <div class="expense-form__group" style="flex:1;min-width:250px;">
                            ...
                            <div class="ws-balance-preview" id="wsBalancePreview" style="display:none;">
                                ...
                                <span class="bp-text">
                                    <span id="bpLabel">Số dư hiện tại:</span>
                                    <strong class="bp-amount" id="bpCurrent">0đ</strong>
                                    <span id="bpArrow" style="display:none;"> → </span>
                                    <strong class="bp-amount" id="bpRemaining" style="display:none;">0đ</strong>
                                </span>
                            </div>
                        </div>
                    </div>
        ...
        <div class="suspicious-modal-content">
            ...
            <div class="suspicious-modal-actions">
                <form action="?template=user&action=add" method="POST" class="suspicious-modal-form">
                    ...
                    <?php if (!empty($suspiciousFormData['use_savings'])): ?><input type="hidden" name="use_savings" value="1"><?php endif; ?>
    ...
    <div class="confirm-popup-overlay">
        <div class="confirm-popup-content">
            ...
            <div class="transaction-details">
                ...
                <?php if (!empty($transactionSuccess['wallet_name'])): ?>
                <div class="detail-row"><span class="detail-label">VÍ</span><span class="detail-value"><?= htmlspecialchars($transactionSuccess['wallet_name']) ?></span></div>
                <?php endif; ?>
            </div>
...
?>
    <div id="bailoutModal" class="suspicious-modal-overlay">
        <div class="suspicious-modal-content bailout-content">
            ...
            <div class="bailout-wallet-list">
                ...
                ?>
                <form action="?template=user&action=add" method="POST" style="margin:0;" class="bailout-form">
                    ...
                    <button type="button" name="add_btn" class="bailout-wallet-item" style="width:100%;text-align:left;" onclick="confirmBailoutWallet(this)">
            ...
            <form action="?template=user&action=add" method="POST" id="multiBailoutForm">
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px;padding:0 4px;">
                    ...
                    <button type="submit" name="add_btn" id="multiSubmitBtn" class="bailout-confirm-btn" style="opacity:0.5;pointer-events:none;" disabled>Thanh toán</button>
                </div>
            </form>
            <?php endif; ?>

            <button onclick="document.getElementById('bailoutModal').style.display='none'" class="bailout-cancel-btn">Hủy giao dịch</button>
        </div>
    </div>

    <!-- Bailout confirmation overlay (outside bailout modal for correct stacking) -->
...
function updateBalancePreview(walletId) {
    ...
    if (walletId) {
        ...
        bpRemaining.style.display = 'inline';
        bpArrow.style.display = 'inline';
        bpLabel.textContent = 'Số dư hiện tại:';
        document.querySelector('#wsBalancePreview .bp-icon').textContent = remaining >= 0 ? 'check_circle' : 'warning';
        if (remaining >= 0) {
            preview.className = 'ws-balance-preview';
            bpRemaining.textContent = numberFormat(remaining) + 'đ';
            bpRemaining.style.color = '#166534';
        } else {
            preview.className = 'ws-balance-preview warning';
            bpRemaining.textContent = '-' + numberFormat(Math.abs(remaining)) + 'đ (sẽ âm)';
            bpRemaining.style.color = '#dc2626';
        }
    } else {
        preview.style.display = 'none';
        bpRemaining.style.display = 'none';
        bpArrow.style.display = 'none';
    }
}
