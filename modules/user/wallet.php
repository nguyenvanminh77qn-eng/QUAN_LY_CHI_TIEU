...
function ajaxResponse($data, $code = 200) {
...
if (isset($_POST['save_wallet'])) {
    ...
    if (empty($name)) {
        if ($isAjax) ajaxResponse(['success'=>false, 'message'=>'Tên ví không được để trống.']);
    ...
    if ($id > 0) {
        ...
        if (!$existing) {
            if ($isAjax) ajaxResponse(['success'=>false, 'message'=>'Ví không tồn tại.']);
            setMessage('Ví không tồn tại.','error'); redirect('?template=user&action=wallet&noanim=1');
        }
        update('wallet', ['name'=>$name, 'icon'=>$icon, 'type'=>$type], "id = :id", ['id'=>$id]);
        if ($isAjax) {
            $allWallets = getWallets($userId);
            $gt = 0;
            foreach ($allWallets as $w) $gt += getWalletBalance($w['id'], $userId);
            ajaxResponse(['success'=>true, 'message'=>'Đã cập nhật ví.','wallet'=>['id'=>$id,'name'=>$name,'icon'=>$icon,'type'=>$type,'balance'=>getWalletBalance($id,$userId)],'grand_total'=>$gt]);
        }
        ...
        if (!$newId) {
            if ($isAjax) ajaxResponse(['success'=>false, 'message'=>'Không thể tạo ví. Vui lòng thử lại.']);
        ...
        if ($isAjax) {
            ...
            $gt = 0;
            foreach ($allWallets as $w) $gt += getWalletBalance($w['id'], $userId);
            ajaxResponse(['success'=>true, 'message'=>'Đã thêm ví mới.','wallet'=>['id'=>$newId,'name'=>$name,'icon'=>$icon,'type'=>$type,'balance'=>0],'grand_total'=>$gt]);
        }
...
if (isset($_POST['delete_wallet'])) {
    ...
    if ($wallet['is_default']) {
        if ($isAjax) ajaxResponse(['success'=>false, 'message'=>'Không thể xóa ví mặc định.']);
    ...
    if ($txCount && $txCount['cnt'] > 0) {
        if ($isAjax) ajaxResponse(['success'=>false, 'message'=>'Không thể xóa ví đã có giao dịch.']);
        setMessage('Không thể xóa ví đã có giao dịch.','error'); redirect('?template=user&action=wallet&noanim=1');
    }

    delete('wallet', "id = :id", ['id'=>$id]);
    if ($isAjax) {
        $allWallets = getWallets($userId);
        $gt = 0;
        foreach ($allWallets as $w) $gt += getWalletBalance($w['id'], $userId);
        ajaxResponse(['success'=>true, 'message'=>'Đã xóa ví.','deleted_id'=>$id,'grand_total'=>$gt]);
    }
