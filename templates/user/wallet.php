...
document.getElementById('walletForm').addEventListener('submit', function(e) {
    ...
    .then(function(res) {
        if (res.success) {
            closeModal();
            if (res.wallet && res.wallet.id) {
                var card = document.querySelector('.wallet-card[data-wallet-id="' + res.wallet.id + '"]');
                if (card) {
                    card.querySelector('.wallet-card-icon-wrap').textContent = res.wallet.icon;
                    card.querySelector('.wallet-card-name').textContent = res.wallet.name;
                    card.className = 'wallet-card type-' + res.wallet.type + ' reveal';
                    var balEl = card.querySelector('.wallet-card-balance');
                    if (res.wallet.balance !== undefined) {
                        balEl.textContent = res.wallet.balance.toLocaleString('vi-VN') + 'đ';
                        balEl.className = 'wallet-card-balance ' + (res.wallet.balance >= 0 ? 'pos' : 'neg');
                    }
                }
            }
            if (res.grand_total !== undefined) updateHeroTotal(res.grand_total);
