...
if (isset($_GET['ajax']) && $_GET['ajax'] === '1') {
    ...
    $limit = 5;
    $totalTransactions = (int)getOne(
        "SELECT COUNT(*) FROM (SELECT 1 FROM transaction WHERE user_id = :uid1 AND is_archived = 0 AND source_type != 'transfer' AND batch_id IS NULL UNION ALL SELECT 1 FROM transaction WHERE user_id = :uid2 AND is_archived = 0 AND source_type != 'transfer' AND batch_id IS NOT NULL GROUP BY batch_id) t",
        ['uid1' => $userId, 'uid2' => $userId]
    )['COUNT(*)'] ?? 0;
    $pagination = getPagination($totalTransactions, $limit, $page);
    ...
    echo json_encode([
        'rows' => $rowsHtml,
        'pagination' => renderPagination($pagination['totalPages'], $pagination['currentPage'], "?template=user&action=dashboard"),
